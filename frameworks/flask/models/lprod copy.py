...
class Lprod_model :
    ...
    def getList_DictType_FetchAll(self, cols, rows) :
        list_rows = []
        for columns in rows :
            dict_row = {}
    ...
    ### 딕셔너리로 데이터 구성하기..
    def getDictType_FetchOne(self, cols, one_row) :
        dict_row = {}    
    ...
    def getLprodList(self) :
        ...
        ### 컬럼명 추출하기
        cols = self.getColumns(cursor.description)
        
        ### MySQL 객체 반납하기
        self.dbClose(cursor, conn)
        
        ### [여러건 조회인 경우] 리스트 + 딕셔너리 형태로 만들기
        list_rows = self.getList_DictType_FetchAll(cols, rows)
        
        return list_rows    
        
    ### lprod 테이블 [상세 조회]하기
    def getLprodView(self, p_lprod_gu) :
        ### Database connection 및 cursor 객체 받아오기
        conn, cursor = self.getConnection()
        ...
        dict_row = self.getDictType_FetchOne(cols, one_row)
        
        return dict_row


    ### lprod 테이블 [입력]하기
    def setLprodInsert(self, p_params) :
        ### Database connection 및 cursor 객체 받아오기
        conn, cursor = self.getConnection()
    ...
    ### lprod 테이블 [수정]하기
    def setLprodUpdate(self, p_params) :
        ### Database connection 및 cursor 객체 받아오기
        conn, cursor = self.getConnection()
    ...
    ### lprod 테이블 [삭제]하기
    def setLprodDelete(self, p_lprod_gu) :
        ### Database connection 및 cursor 객체 받아오기
        conn, cursor = self.getConnection()
